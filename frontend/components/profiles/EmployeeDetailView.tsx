...
const SatisfactionBar: React.FC<{
  label: string;
  value: number | undefined;
}> = ({ label, value = 0 }) => {
  const color =
    value < 60 ? "bg-red-500" : value < 80 ? "bg-yellow-500" : "bg-green-500";
  return (
    <div>
...
> = {
  ...
  Proficient: {
    label: "Proficient",
    colorClasses: "bg-purple-800 text-purple-200",
  },
...
const EmployeeDetailView: React.FC<EmployeeDetailViewProps> = ({
  ...
  const flightRiskScore = useMemo(
    () => calculateFlightRiskScore(employee),
    [employee]
  );
  ...
  const sortedSkills = useMemo(() => {
    ...
    return [...employee.skills].sort(
      (a, b) => skillOrder.indexOf(b.skillLevel) - skillOrder.indexOf(a.skillLevel)
    );
  }, [employee.skills]);
    ...
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row items-center gap-6 p-6 bg-card rounded-lg border border-border bg-gradient-to-br from-card via-card to-primary-900/30">
        <div
          className={`w-24 h-24 rounded-full ${getAvatarColor(
            employee.gender
          )} flex items-center justify-center font-bold text-white text-5xl flex-shrink-0`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <InfoPill
          icon={Clock}
          label="Tenure"
          value={calculateTenure(employee.hireDate, employee.terminationDate)}
        />
        <InfoPill
          icon={Award}
          label="Salary"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="flex flex-wrap justify-around items-center gap-y-6 pt-4">
            {/* 1. PERFORMANCE (Already Correct) */}
            <Gauge
              value={(employee.performanceRating || 0) * 20} // Convert 0-5 scale to 0-100 for gauge
              label="Performance"
              displayValue={(employee.performanceRating || 0).toFixed(1)}
              displayMaxValue="/5"
            />

            {/* 2. ENGAGEMENT (Use the field from backend we verified is 64) */}
            <Gauge
              value={employee.engagementScore || 0}
              label="Engagement"
              displayValue={(employee.engagementScore || 0).toFixed(0)}
              displayMaxValue="%"
            />

            {/* 3. POTENTIAL (Already Correct) */}
            <Gauge
              value={(((employee.potentialRating || 1) - 1) / 2) * 100}
              label="Potential"
              displayValue={(employee.potentialRating || 0).toString()}
              displayMaxValue="/3"
            />

            {/* 4. FLIGHT RISK (Backend or Fallback) */}
            {/* If you want to use the backend value, ensure it is mapped in DataContext. 
        Currently, you calculate it locally. If that's intentional, keep it. 
        If you want backend data: value={(employee.flightRiskScore || 0) * 10} */}
            <Gauge
            ...
            {/* 5. IMPACT (Backend or Fallback) */}
            {/* If you mapped backend 'impactScore' to 'engagementScore', 
        this might be redundant, or you can map it separately in DataContext. */}
            <Gauge
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-text-primary">
                  {employee.successionStatus}
                </span>
              </div>
              <p className="text-sm text-text-secondary mt-2">
                Employee's readiness for promotion to key roles.
              </p>
            </CardContent>
          </Card>
          <Card>
              ...
              <div className="flex flex-wrap gap-2">
                {sortedSkills.length > 0 ? (
                  sortedSkills.map((skill) => (
                    <span
                      key={skill.skillLevelId}
                      className={`px-3 py-1 text-sm font-medium rounded-full ${
                        skillLevelConfig[skill.skillLevel].colorClasses
                      }`}
                    >
                      {skill.skillName}
                    </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between text-sm font-medium text-text-secondary">
                ...
                <span>
                  {employee.trainingCompleted} / {employee.trainingTotal}{" "}
                  modules
                </span>
              </div>
            ...
            {employee.trainingCompleted > 0 && (
              <div>
                ...
                <ul className="space-y-1 text-sm text-text-secondary list-disc list-inside">
                  {mockTrainingModules
                    .slice(0, employee.trainingCompleted)
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <SatisfactionBar
              label="Compensation"
              value={employee.compensationSatisfaction}
            />
            <SatisfactionBar
              label="Benefits"
              value={employee.benefitsSatisfaction}
            />
            <SatisfactionBar
              label="Management"
              value={employee.managementSatisfaction}
            />
            <SatisfactionBar
              label="Training"
              value={employee.trainingSatisfaction}
            />
          </CardContent>
        </Card>
        <Card>
              ...
              value={
                <span
                  className={`flex items-center gap-2 font-semibold ${
                    employee.hasGrievance ? "text-red-400" : "text-text-primary"
                  }`}
                >
